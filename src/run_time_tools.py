...
def mf_with_bias(config, lr=1e-2, l2_factor=1e-2, max_step=1000, train_rate=0.95, max_stop_count=30):
    ...
    user_embeddings = tf.Variable(tf.truncated_normal(
        [user_num, emb_size], mean=0, stddev=0.01))
    item_embeddings = tf.Variable(tf.truncated_normal(
        [item_num, emb_size], mean=0, stddev=0.01))
    ...
    loss = target_loss + l2_factor * \
        (tf.reduce_mean(tf.square(user_embs) + tf.square(item_embs)))
    ...
    with tf.Session() as sess:
        ...
        rmse_train, loss_v, target_loss_v = sess.run([rmse, loss, target_loss], feed_dict={
                                                     user_ids: dtrain[:, 0], item_ids: dtrain[:, 1], ys: np.float32(dtrain[:, 2])})
        rmse_test = sess.run(rmse, feed_dict={
                             user_ids: dtest[:, 0], item_ids: dtest[:, 1], ys: np.float32(dtest[:, 2])})
        ...
        np.savetxt('../data/run_time/'+rating_file+'_item_embedding_dim%d' %
                   emb_size, delimiter='\t', X=item_embeddings_value)
...
def clustering_vector_constructor(config, sess, max_user_num=30000):
    ...
    result_file_path = '../data/run_time/%s_%s_vector_v%d' % (
        config['ENV']['RATING_FILE'], config['TPGR']['CLUSTERING_VECTOR_TYPE'].lower(), int(config['TPGR']['CLUSTERING_VECTOR_VERSION']))

    if config['TPGR']['CLUSTERING_VECTOR_TYPE'] == 'RATING':
        rating_matrix = cur_env.get_init_data()[2].toarray()[:max_user_num]
        np.savetxt(X=rating_matrix.transpose(),
                   fname=result_file_path, delimiter='\t')
    elif config['TPGR']['CLUSTERING_VECTOR_TYPE'] == 'MF':
        np.savetxt(X=np.loadtxt(fname='../data/run_time/%s_item_embedding_dim%s' %
                                (config['ENV']['RATING_FILE'], config['META']['ACTION_DIM']), delimiter='\t'), fname=result_file_path, delimiter='\t')
    elif config['TPGR']['CLUSTERING_VECTOR_TYPE'] == 'VAE':
        rating_matrix = cur_env.get_init_data()[2].toarray()[:max_user_num]
        vae = VAE(rating_matrix.transpose(), sess,
                  h2_size=int(config['META']['ACTION_DIM']))
        result = vae.run()
...
class VAE():
    def __init__(self, data, sess, kl_factor=5e-4, learning_rate=2e-3, h1_size=128, h2_size=8, max_step=1000):
        ...
        self.input = tf.placeholder(
            shape=[None, self.data_dim], dtype=tf.float32)
        self.epsilon = tf.placeholder(
            shape=[None, self.h2_size], dtype=tf.float32)

    def construct_encoder(self):
        W1 = tf.Variable(tf.random_normal(
            shape=[self.data_dim, self.h1_size], stddev=0.01))
        W2 = tf.Variable(tf.random_normal(
            shape=[self.h1_size, self.h2_size*2], stddev=0.01))
        ...
        self.mean = tf.nn.relu(
            tf.add(tf.matmul(h1, W2[:, :self.h2_size]), b2[:self.h2_size]))
        self.stddev = tf.nn.relu(
            tf.add(tf.matmul(h1, W2[:, self.h2_size:]), b2[self.h2_size:]))

    def construct_decoder(self):
        W1 = tf.Variable(tf.random_normal(
            shape=[self.h2_size, self.h1_size], stddev=0.01))
        W2 = tf.Variable(tf.random_normal(
            shape=[self.h1_size, self.data_dim], stddev=0.01))
        b1 = tf.Variable(tf.zeros(shape=[self.h1_size]))
    ...
    def make_graph(self):
        ...
        self.KL_loss = -tf.reduce_mean(1.0 + 2.0 * tf.log(
            self.stddev+1e-10) - tf.square(self.stddev) - tf.square(self.mean), 1)
        self.reconstruction_loss = 0.5 * \
            tf.reduce_mean(tf.square(self.input-self.output), 1)
    ...
    def run(self, max_stop_count=100):
        ...
        for step in range(self.max_step):
            ...
            _, loss, kl_loss, rec_loss = self.sess.run([self.train_op, tf.reduce_mean(self.loss), tf.reduce_mean(self.KL_loss), tf.reduce_mean(self.reconstruction_loss)], feed_dict={
                                                       self.input: self.data[index[:1000]], self.epsilon: np.random.normal(size=[len(self.data[index[:1000]]), self.h2_size])})
            rmse = self.sess.run(tf.reduce_mean(self.reconstruction_loss), feed_dict={
                                 self.input: self.data[index[:1000]], self.epsilon: np.zeros(shape=[len(self.data[index[:1000]]), self.h2_size])})
            print('step %3d, loss %f, rec %.5f, rmse %.5f, kl %.5f' %
                  (step, loss, rec_loss, rmse, kl_loss))
            if pre_loss < loss:
                stop_count += 1
                if stop_count == max_stop_count:
                    break
        ...
        result = [self.sess.run(self.mean, feed_dict={
                                self.input: self.data[i*1000:(i+1)*1000]}) for i in range(math.ceil(len(self.data)/1000))]

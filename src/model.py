...
class PRE_TRAIN():
    def __init__(self, config, sess):
        ...
        self.learning_rate = float(
            self.config['TPGR']['PRE_TRAINING_LEARNING_RATE'])
        self.l2_factor = float(self.config['TPGR']['PRE_TRAINING_L2_FACTOR'])
        self.pre_train_truncated_length = int(
            self.config['TPGR']['PRE_TRAINING_RNN_TRUNCATED_LENGTH'])
        self.max_item_num = int(
            self.config['TPGR']['PRE_TRAINING_MAX_ITEM_NUM'])
        self.pre_train_seq_length = min(
            int(self.config['TPGR']['PRE_TRAINING_SEQ_LENGTH']), self.max_item_num)
        self.pre_train_mask_length = min(
            int(self.config['TPGR']['PRE_TRAINING_MASK_LENGTH']), self.pre_train_seq_length)
    ...
    def make_graph(self):
        ...
        self.pre_statistic = [tf.placeholder(dtype=tf.float32, shape=[
                                             None, self.statistic_dim]) for i in range(self.pre_train_truncated_length)]
        self.pre_rnn_state = tf.placeholder(
            dtype=tf.float32, shape=[2, None, self.rnn_output_dim], name='pre_rnn_state')
        ...
        # rnn
        self.initial_states = tf.stack([tf.zeros([self.batch_size, self.rnn_output_dim]), tf.zeros([
                                       self.batch_size, self.rnn_output_dim])])
        self.rnn, self.rnn_variables = self.create_sru(
            self.rnn_input_dim, self.rnn_output_dim)
        ...
        # pre-train operation
        self.W = tf.Variable(self.init_matrix(
            shape=[self.rnn_output_dim + self.statistic_dim, self.max_item_num]))
        self.b = tf.Variable(self.init_matrix(shape=[self.max_item_num]))
        self.l2_norm = tf.nn.l2_loss(self.W) + tf.nn.l2_loss(self.b)
        self.pn_outputs = [tf.matmul(tf.concat([self.cur_rnn_states_list[i][0], self.pre_statistic[i]],
                                               axis=1), self.W) + self.b for i in range(self.pre_train_truncated_length)]
        self.expected_pn_outputs = tf.placeholder(
            dtype=tf.float32, shape=[None, self.max_item_num])
        self.mask = tf.placeholder(dtype=tf.float32, shape=[
                                   None, self.max_item_num])
        self.all_zero_loss = tf.pow((1.0 / tf.reduce_sum(self.mask)) * tf.reduce_sum(tf.square(self.mask * (tf.cast(
            tf.constant(np.zeros(shape=[self.batch_size, self.max_item_num])), dtype=tf.float32) - self.expected_pn_outputs))), 0.5)
        self.loss = [tf.pow((1.0 / tf.reduce_sum(self.mask)) * tf.reduce_sum(tf.square(self.mask * (
            self.pn_outputs[i] - self.expected_pn_outputs))), 0.5) for i in range(self.pre_train_truncated_length)]
        self.pre_train_op = [tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(
            self.loss[i] + self.l2_norm * self.l2_factor) for i in range(self.pre_train_truncated_length)]
    ...
    def train(self):
        ...
        for i in range(self.pre_train_seq_length):      # Sample actions, just train rnn
        ...
        mask_value = np.zeros(dtype=float, shape=[
                              self.batch_size, self.max_item_num])
        ...
        if self.pre_training_steps % self.log_step == 0:
            self.evaluate()
            utils.pickle_save(self.sess.run(self.rnn_variables),
                              self.rnn_file_path + 's%d' % self.pre_training_steps)
    ...
    def evaluate(self):
        ...
        for i in range(self.pre_train_seq_length):
            ...
            print('all zero rmse:%.3f, rnn rmse:%.3f, l2:%.3f' % tuple(self.sess.run(
                [self.all_zero_loss, self.loss[0], self.l2_norm], feed_dict=feed_dict)))
...
class Tree():
    def __init__(self, config, sess):
        ...
        self.clustering_vector_file_path = '../data/run_time/%s_%s_vector_v%s' % (
            config['ENV']['RATING_FILE'], self.config['TPGR']['CLUSTERING_VECTOR_TYPE'].lower(), self.config['TPGR']['CLUSTERING_VECTOR_VERSION'])
        self.tree_file_path = '../data/run_time/%s_tree_model_%s_%s_c%d_%s' % (self.config['ENV']['RATING_FILE'], self.config['TPGR']['CLUSTERING_VECTOR_TYPE'].lower(
    ...
    def kmeans_clustering(self, item_list, id_to_vector):
        ...
        reordered_index = [item[0] for item in sorted(
            index2len, key=lambda x: x[1], reverse=True)]
        ...
        non_decrease_num = self.child_num - \
            (self.child_num * list_len - len(item_list))
        target_len = [list_len for i in range(
            non_decrease_num)] + [list_len - 1 for i in range(self.child_num - non_decrease_num)]
        ...
        for i in range(self.child_num):
            ...
            if num > 0:
                ...
                top_n_p_dis_pairs = sorted(
                    p_dis_pairs, key=lambda x: x[1])[:num]
    ...
    def pca_clustering(self, item_list, id_to_vector):
        ...
        pca.fit(data)
        w = pca.components_[0]
        item_to_projection = [(item, np.dot(id_to_vector[item], w))
        ...
        non_decrease_num = self.child_num - \
            (self.child_num * list_len - len(result))
        start = 0
        end = list_len
        for i in range(self.child_num):
    ...
    def hierarchical_code(self, item_list, code_range, id_to_code, id_to_vector):
        ...
        for i in range(self.child_num):
            self.hierarchical_code(item_list_assign[i], (
                code_range[0]+i*range_len, code_range[0]+(i+1)*range_len), id_to_code, id_to_vector)
...
class TPGR():
    def __init__(self, config, sess):
        ...
        self.sample_episodes_per_batch = int(
            self.config['TPGR']['SAMPLE_EPISODES_PER_BATCH'])
        self.sample_users_per_batch = int(
            self.config['TPGR']['SAMPLE_USERS_PER_BATCH'])
        ...
        self.result_file_path = '../data/result/result_log/' + \
            ...
            self.config['ENV']['ALPHA'] + '_' + \
            self.config['ENV']['RATING_FILE']
        self.rnn_file_path = '../data/run_time/%s_rnn_model_%s' % (
            self.config['ENV']['RATING_FILE'], self.config['TPGR']['RNN_MODEL_VS'])
        self.load_model = self.config['TPGR']['LOAD_MODEL'] == 'T'
        self.load_model_path = '../data/model/%s_tpgr_model_%s' % (
            self.config['ENV']['RATING_FILE'], self.config['TPGR']['MODEL_LOAD_VS'])
        self.save_model_path = '../data/model/%s_tpgr_model_%s' % (
            self.config['ENV']['RATING_FILE'], self.config['TPGR']['MODEL_SAVE_VS'].split('s')[0])
        self.tree_file_path = '../data/run_time/%s_tree_model_%s_%s_c%d_%s' % (self.config['ENV']['RATING_FILE'], self.config['TPGR']['CLUSTERING_VECTOR_TYPE'].lower(
        ), self.config['TPGR']['CLUSTERING_TYPE'].lower(), self.child_num, self.config['TPGR']['TREE_VS'])
        self.hidden_units = [int(item) for item in self.config['TPGR']['HIDDEN_UNITS'].split(
            ',')] if self.config['TPGR']['HIDDEN_UNITS'].lower() != 'none' else []

        # use self.forward_env.item_genres instead
        # # read movies' genres file
        # genre_file_path = '../data/rating/' + self.config['ENV']['GENRE_FILE']
        # self.item_genres = utils.read_genre_file(genre_file_path)
    ...
    def make_graph(self):
        # placeholders
        self.forward_action = tf.placeholder(
        ...
        self.forward_statistic = tf.placeholder(
            dtype=tf.float32, shape=[None, self.statistic_dim], name='forward_statistic')
        self.forward_rnn_state = tf.placeholder(dtype=tf.float32, shape=[
                                                2, None, self.rnn_output_dim], name='forward_rnn_state')
        self.cur_q = tf.placeholder(
            dtype=tf.float32, shape=[None], name='cur_qs')
        self.cur_action = tf.placeholder(
            dtype=tf.int32, shape=[None], name='cur_actions')
        ...
        # RNN input
        self.forward_a_emb = tf.nn.embedding_lookup(
            self.action_embeddings, self.forward_action)
        one_hot_reward = tf.one_hot(tf.cast(
            (self.reward_dim*(1.0-self.forward_reward)/2), dtype=tf.int32), depth=self.reward_dim)
        self.forward_ars = tf.concat(
            [self.forward_a_emb, one_hot_reward, self.forward_statistic], axis=1)

        # RNN initial state
        self.initial_states = tf.stack([tf.zeros([self.train_batch_size, self.rnn_output_dim]), tf.zeros([
                                       self.train_batch_size, self.rnn_output_dim])])
        ...
        self.user_state = tf.concat(
            [self.rnn_state[0], self.forward_statistic], axis=1)

        if self.load_model:
            model = utils.pickle_load(self.load_model_path)
            self.W_list = [tf.Variable(model['W_list'][i], dtype=tf.float32)
                           for i in range(len(model['W_list']))]
            self.b_list = [tf.Variable(model['b_list'][i], dtype=tf.float32)
                           for i in range(len(model['b_list']))]
        ...
        self.aval_list = tf.Variable(np.tile(np.expand_dims(self.aval_val, 1), [
                                     1, self.train_batch_size, 1]), dtype=tf.float32)
        self.aval_eval_list = tf.Variable(np.tile(np.expand_dims(self.aval_val, 1), [
                                          1, self.eval_batch_size, 1]), dtype=tf.float32)
        self.eval_probs = []
        ...
        for i in range(self.bc_dim):
            self.forward_index = self.node_num_before_depth_i(
                i) + self.child_num * self.pre_shift + tf.cast(self.pre_mul_choice, tf.int32)
            if i == 0:
                h = self.user_state
            else:
    ...
    def train(self):
        ...
        for i in range(self.sample_users_per_batch):
            qs[:, i * self.sample_episodes_per_batch: (i + 1) * self.sample_episodes_per_batch] = self.standardization(
    ...
    def evaluate(self):
        ...
        for s in range(eval_step_num):
            ...
            rnn_state = np.zeros(
                [2, self.eval_batch_size, self.rnn_output_dim])
            step_count = 0
            stop_flag = False
            entropy_list.append([])
            while True:
                ...
                rmse = np.power(
                    np.mean(np.square(probs - 1.0 / self.child_num)), 0.5)
        ...
        for j in range(self.user_num):
            ...
            ratings = [self.forward_env.get_rating(
                ars[0][k][j]) for k in range(0, len(ars[0]))]
        ...
        test_ave_precision = np.mean(
            precision[self.boundry_user_id:self.user_num])
        ...
        utils.pickle_save(model, self.save_model_path +
                          's%d' % self.training_steps)

        # save the result
        self.storage.append([train_ave_reward, train_ave_precision, train_ave_recall, train_ave_f1,
                             test_ave_reward, test_ave_precision, test_ave_recall, test_ave_f1, ave_rmse])
        utils.pickle_save(self.storage, self.result_file_path)

        print('training step: %d' % (self.training_steps))
        print('\ttrain average reward over step: %2.4f, precision@%d: %.4f, recall@%d: %.4f, f1@%d: %.4f' % (train_ave_reward,
                                                                                                             self.episode_length, train_ave_precision, self.episode_length, train_ave_recall, self.episode_length, train_ave_f1))
        print('\ttest  average reward over step: %2.4f, precision@%d: %.4f, recall@%d: %.4f, f1@%d: %.4f' % (test_ave_reward,
                                                                                                             self.episode_length, test_ave_precision, self.episode_length, test_ave_recall, self.episode_length, test_ave_f1))
...
class GRLRS():
    def __init__(self, config, sess):
        ...
        self.episode_length = int(self.config['META']['EPISODE_LENGTH'])
        self.action_dim = int(self.config['META']['ACTION_DIM'])
        self.genre_dim = int(self.config['GENRE']['GENRE_COUNT'])   # DEBUG0
        self.statistic_dim = int(self.config['META']['STATISTIC_DIM'])
        self.reward_dim = int(self.config['META']['REWARD_DIM'])
        self.discount_factor = float(self.config['META']['DISCOUNT_FACTOR'])
        self.log_step = int(self.config['META']['LOG_STEP'])
        ...
        self.load_model = self.config['TPGR']['LOAD_MODEL'] == 'T'
        self.load_model_path = '../data/model/%s_tpgr_model_%s' % (
            code_name, self.config['TPGR']['MODEL_LOAD_VS'])
        self.save_model_path = '../data/model/%s_tpgr_model_%s' % (
            code_name, self.config['TPGR']['MODEL_SAVE_VS'].split('s')[0])
        self.tree_file_path = '../data/run_time/%s_tree_model_%s_%s_c%d_%s' % (code_name, self.config['TPGR']['CLUSTERING_VECTOR_TYPE'].lower(
        ), self.config['TPGR']['CLUSTERING_TYPE'].lower(), self.child_num, self.config['TPGR']['TREE_VS'])
        self.hidden_units = [int(item) for item in self.config['TPGR']['HIDDEN_UNITS'].split(
            ',')] if self.config['TPGR']['HIDDEN_UNITS'].lower() != 'none' else []
    ...
    def make_graph(self):
        ...
        self.forward_a_emb = tf.nn.embedding_lookup(            # forward action embedding
            self.action_embeddings, self.forward_action)
        one_hot_genre = tf.one_hot(self.forward_genre, depth=self.forward_env.genre_cnt)        # DEBUG0: forward_genre range from [0,genre_cnt-1]
        one_hot_reward = tf.one_hot(tf.cast(
            (self.reward_dim*(1.0-self.forward_reward)/2), dtype=tf.int32), depth=self.reward_dim)
        self.forward_ars = tf.concat(                           # forward [action, reward, statistic], for rnn use (state representation)
            [self.forward_a_emb, one_hot_genre, one_hot_reward, self.forward_statistic], axis=1)    # DEBUG0
        ...
        if self.load_model: # TODO: check how it should be
            model = utils.pickle_load(self.load_model_path)
            self.W_list = [tf.Variable(model['W_list'][i], dtype=tf.float32)
                           for i in range(len(model['W_list']))]
            self.b_list = [tf.Variable(model['b_list'][i], dtype=tf.float32)
